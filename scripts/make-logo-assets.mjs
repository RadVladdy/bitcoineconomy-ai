...
// public/favicon.svg — the fallback a browser fetches on any route that doesn't
// carry Base.astro's <link rel="icon">. Until 2026-08-10 it was a hand-written
// bitcoin text glyph with no robot: the wrong mark entirely. Wrapping the 180px
// render keeps it identical to the tab icon at every size.
const b64 = (await fs.readFile(path.join(OUT, 'apple-touch.png'))).toString('base64');
await fs.writeFile(
  path.resolve(__dirname, '..', 'public', 'favicon.svg'),
  `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 180 180" role="img" aria-label="bitcoineconomy.ai">
  <!-- The site's logo mark (robot head behind a bitcoin symbol). Generated by
       scripts/make-logo-assets.mjs - do not hand-edit. -->
  <image width="180" height="180" xlink:href="data:image/png;base64,${b64}"/>
</svg>
`,
);

console.log('Wrote logo-mark.png, favicon-32.png, apple-touch.png, og-default.png, favicon.svg');
