  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        expires_in_days: { type: 'number', description: 'Adds a NIP-40 expiration tag — "stop showing this after N days". Past that point the relays stop serving the request at all, so it DISAPPEARS from the board rather than showing as expired. It is not a cancel button either: a relay refuses an event whose expiration is already past, so you cannot retire a live request by back-dating one. To end a request early, republish it under the SAME request_id with status "withdrawn" — it stays visible and readable that way. Omit for no expiry.' },
