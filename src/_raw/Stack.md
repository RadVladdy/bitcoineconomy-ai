...
The standardized way to acquire that inbound capacity is a **Lightning Service Provider (LSP)** — a peer that opens a channel to the agent on demand, under an open spec (LSPS1 to order a channel outright, LSPS2 for just-in-time channels paid out of the first incoming payment) so a wallet can switch providers without code changes. Where an agent wants a *market* rather than a single provider, **liquidity marketplaces** match buyers and sellers of channel liquidity: [[amboss|Amboss Magma]] is the deployed instance — buy inbound capacity from a public market with one API call, paid in sats — whereas [[loop|Loop]] moves an agent's *own* balance between Lightning and L1. Acquiring liquidity is solved engineering, not an open problem; buying and selling it is a *marketplace* activity — the venues (Magma to buy, [[rails|Rails]] to sell capacity for yield) live in [The Market](/services), and the full set sits under the liquidity category in [the directory](https://marketplace.bitcoineconomy.ai).
...
**[[fedimint|Fedimint]]** — federated-mint trust model. A federation of guardians (minimum 4, commonly 4/7/10, capped at 20) jointly custodies the Lightning balance backing issued ecash via threshold signatures; ecash issuance and redemption require federation consensus; no single guardian can defect or be compromised in isolation. Lightning interoperability runs through gateway nodes that interface between the federation and the wider Lightning Network.
...
**Newer L2/L3 scaling layers.** **Ark** — covenant-based shared-UTXO scaling proposal; trust-minimized; allows multiple users to share a single UTXO via covenant-enforced exit conditions; earlier-stage as of mid-2026. **[[spark|Spark]]** — a shared-UTXO, Lightning-compatible L2 built by Lightspark; on mainnet (beta) since May 2025 with three Spark Operators (Lightspark, Flashnet, and Breez — grown from two) and a Q2 2026 roadmap. **[[xverse-agent-wallet|Xverse Agent Wallet]]** uses Spark for agent-facing Lightning settlement (see section 5). The empirical adoption record defers to [[Field-Notes]] — Active developments.
...
**Paying for the tool call itself: CEP-8.** The L402 story has a rhyme one layer up. HTTP left `402 Payment Required` reserved and empty for thirty years because there was no internet-native money to settle it; Lightning finally filled it in. [[mcp|MCP]] — the standard way agents reach tools — is younger: it shipped in late 2024 with no payment semantics at all. But this time the money already existed, so there was no long wait. The empty slot got filled almost immediately, by whoever needed it first. **[[contextvm|ContextVM]]** carries MCP over [[nostr|Nostr]], and its **CEP-8** spec is that fill: it lets an MCP server charge sats for a tool call the way L402 charges for an HTTP resource. The two gates are the same shape:

| | L402 | CEP-8 explicit gating |
|---|---|---|
| Signal | `HTTP 402 Payment Required` | JSON-RPC `-32042 Payment Required` |
| Credential | macaroon + preimage | a paid execution authorization, keyed to the client's pubkey + a hash of the invocation |
| Retry | same request + `Authorization: L402 <macaroon>:<preimage>` | same tool + arguments; the request's id may differ |
| Pending state | (none specified) | `-32043 Payment Pending` + `retry_after` |
| What's gated | an HTTP resource | the tool call |

A server advertises a price as a tag on its capability announcement — `["cap", "tool:get_weather", "100", "sats"]` — so what a tool *does* and what it *costs* arrive in the same signed event (discoverable from the command line with [[cvmi|cvmi]]). Fiat is allowed as a currency label but its conversion is left implementation-defined; sats is the paved road. CEP-8 has two flows: **`transparent`** (the default — the server quietly waits for payment, then forwards the call) and **`explicit_gating`** (opt-in — the server returns a visible `Payment Required` error the agent can reason about, which is usually what an autonomous caller wants). Payment is handled as a **transport concern**: the underlying tool need not know a payment happened, which is why CEP-8 drops into the wallet architectures below without needing a new one. The recommended rails are Lightning over [[alby-nwc|NWC]] (`bitcoin-lightning-bolt11`) and [[cashu|Cashu]] (`bitcoin-cashu`). How many servers actually speak it in the wild is a [[Field-Notes]] question; the structural point is smaller and harder to argue with — the moment MCP existed, the rail its builders reached for to price it was Bitcoin, not a card. The contest between this pubkey-and-relay path and the DNS-and-registry one is [[Border-Skirmishes]]' ground.
...
The security model runs through every layer of the Stack. Six architectural patterns are operationally significant.
...
**No-inbound-surface serving.** The five patterns above all protect keys *while spending*. A [[contextvm|CVM]] server adds a different axis: it removes the inbound network surface entirely. It has no listening port, no DNS record, no TLS certificate, no API key to leak, and no IP address to scan — it publishes to and subscribes from relays instead. Much of what remote-signer isolation and scoped macaroons exist to defend *behind* simply stops existing. The honest part: you have not deleted the risk, you have moved it. **Relay availability** becomes a liveness dependency — the mitigation, CEP-17 relay lists and publishing to several relays, is the same shape as the multi-watchtower redundancy above. **Metadata** leaks: CEP-4 can encrypt the payload, but which pubkey talks to which, and when, is visible to whoever runs the relay (CEP-19 ephemeral gift wraps address part of this). And **key compromise is now total** — with no IP allowlist or network boundary as a second factor, the keypair is the entire perimeter. The pattern trades a network perimeter for a cryptographic one; for an agent that is usually the right trade, and it is not a free one.
...
The layers and primitives above compose into a handful of recognisable stacks. The first three are points on one spectrum — *how much you run yourself*:
...
A fourth shape sits on a different axis — not *how much you run*, but *how what you run is reachable*. It is the first one here aimed at an agent that wants to **sell** a capability rather than buy one:

- **No-ports sovereign stack** — [[bitcoin-core|Bitcoin Core]] under `lnd`, [[lnbits|LNbits]] for programmable wallets, but the agent-facing surface is a [[contextvm|CVM]] server on a keypair rather than an HTTP endpoint. Maximum sovereignty *and* no exposed inbound surface; in exchange you depend on relays for reachability and on the much smaller population of clients that speak the protocol.
