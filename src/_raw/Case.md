...
**The integration surface is deployed, not hypothetical.** Production agent-payment tooling — the Lightning Agent Toolkit, L402, NWC, Cashu/Fedimint mints, and a growing set of agent-native wallets — ships today; the agent payment stack is something to assemble, not build. The current roster, versions, and capacity are in [[Stack|The Stack]] and [[Field-Notes|Field Notes]].
...
**2026-06-08 — markdown disqualification table CUT (Option A, user).** The "Why the legacy economy fails" section carried *two* tables on the live page, back-to-back: the build-injected **matrix scorecard** (`data-diagram="matrix"`, rows incl. the Bitcoin winner + ✓/◐/✗ legend) and a redundant **markdown disqualification table** (legacy-only, text reasons, no Bitcoin row). They duplicated the same substrate × four-constraints evaluation (and even used different column labels). Resolution: **keep the matrix as the single scorecard; cut the markdown table; fold its per-candidate failure reasons into the prose** (bank rails + credit cards added to the stablecoin/CBDC paragraph; "read the table" → "read the scorecard"). Removes the accidental redundancy. *(Deploy-coupled — rides the next repo-port + push, batched with the m2m→A2A sweep and the matrix mobile-overflow CSS fix.)*

**Open for review:** (1) the **matrix scorecard's mobile rendering** — last column (Machine-tempo) is partially cut off when scrolled right; fix `figure.matrix table` overflow in the repo `src/styles/global.css`. (2) The capacity beat is intentionally short (a paragraph) because The Adoption Asymmetry may end up section-first inside this page rather than its own surface; if it's promoted, this beat stays as the teaser; if it's folded in, this beat grows into the section. (3) Sources was trimmed toward the load-bearing few with the roster pushed to Stack/Field Notes — confirm that's the right depth for the anchor doc.
