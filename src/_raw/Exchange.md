...
- **Non-custodial swaps need no KYC → the agent is sovereign, but crypto-only.** Services like SideSwap and SideShift execute atomic swaps without an account or identity (Boltz did too, until it suspended swaps on 2026-08-03); the agent acts on its own keys and never hands over custody. This is the no-KYC, self-custody ideal — software-manageable without human account intermediation. But these services convert *between crypto assets* (BTC, Lightning, stablecoins, other layers); they do **not** touch bank fiat, and they trade regulatory protection for autonomy (see the caveats below).
...
The agent swaps on its own keys, with no delegated identity. This is the most agent-native path; the caveats below are the price of that sovereignty. *(SideShift is the exception on the account point since 2026-08-07 — its API now needs an auto-created account credential.)* *(Structural facts WebSearch-verified 2026-06-03; Boltz re-verified 2026-06-05; Flashnet + Taproot Assets verified 2026-06-06. ✓ yes · — no · ⚠ limited; the API column is the capability an agent needs to run a swap unattended.)*
...
| **SideShift** | swap | — *(switched off 2026-08-07)* | USDT *(Liquid)* + 200+ assets | ✓ REST *(account credential)* | — |
| **Flashnet** | AMM *(Spark)* | ✓ *(via Spark)* | USDB *(Spark AMM)* · USDT / USDC *(Orchestra API only)* | ✓ skill / API | — |
| **Taproot Assets** | Lightning FX *(edge-node)* | ✓ | USDT *(Lightning)* | ⚠ *(`tapd`, no REST)* | — |

⚠ **Boltz suspended all swap services indefinitely on 2026-08-03** and is no longer a live option — it had been the standout for agents, on the cleanest mix of no-KYC, non-custodial atomicity and a full automation API. Its own notice is worth reading: the shutdown was a response to sustained AI-assisted probing of an open-source stack, and **unilateral refunds kept working because they never depended on Boltz's infrastructure** — the non-custodial design doing precisely what it promises. Its [card](/exchanges/boltz) is retained, unfeatured, for the record. *(Update 2026-08-10: **Blockstream announced Blockstream Swaps** — atomic swaps across Bitcoin L1, Liquid and Lightning, framed explicitly as redundancy for this gap — but it is in closed beta with no public API, so it is not yet an option an agent can route to; the dated record is in [the Log](/field-notes-log).)* **SideSwap** is pure atomic swaps on Liquid (liquidity tracks order-book depth). **SideShift** spans the most assets but is not as clean as the other two: an automated risk-screening layer can flag and hold funds and may demand KYC/source-of-funds to release, its API now requires an auto-created account credential, and **its Lightning leg is switched off** (measured 2026-08-07). **Flashnet** is a non-custodial AMM on Spark (Lightspark's Bitcoin L2) swapping BTC↔USDB with an open-source agent skill — a strong agent fit, though newer and carrying Spark's operator-set trust; its USDT and USDC routes are a separate product, reached through the **Orchestra** API, which needs a key and secret and screens addresses through Elliptic before funds move. **Taproot Assets** turns Lightning itself into a BTC↔USDT FX rail via edge-node swaps — sovereign and no-KYC, but it is *rails* (the stablecoin stays issuer-freezable) and has no clean swap API, so it asks more setup than the others. None reach bank fiat — and that dividing line is the point: there is no no-KYC, **API-driven**, *fiat*-settling exchange, because that is exactly what KYC law exists to prevent. The one no-KYC road to fiat that does exist — Mostro, below — proves the rule by keeping a human on the fiat leg.
...
| **River** | BTC-only | US | ⚠ *(RLS withdrawn ~2026-04)* | — | — / — / — *(no public agent API)* | ✓ |
...
> - **[[Marketplace|The Agent Marketplace]]** — the live directory of services an agent can consume and offer for Bitcoin.
