...
canonical-source: "[[Border-Skirmishes]]"
epistemic-status: "structural argument with live-contest empirical anchor (2026 production stacks: lightning-agent-tools Feb 2026 on the Bitcoin substrate; AgentCore Payments May 2026, Google AP2 consortium, A2A x402 extension, x402 at the Linux Foundation, Circle Nanopayments May 2026, Skyfire, Lightspark Grid on the competing substrate); combatants treated as TYPES; version-specific numbers and rosters defer to Field Notes; forward-looking inferences explicitly tagged"
claims-index:
  - id: BS1
    tag: structural
    statement: "The live competition at the boundary confirms the Independence Doctrine rather than refuting it: faced with the agent economy, the incumbents did not adapt toward Bitcoin's properties; they built parallel competing-substrate agent-payment rails that preserve their own identity-binding, freeze-capable, issuer-controlled property bundle. Building a parallel rail rather than adapting is exactly what the Doctrine's mutual-exclusion mechanism (Doctrine-FA D1) predicts."
    defended-in: "§3, §8.1 CP1"
  - id: BS2
    tag: structural
    statement: "The contest is on the asset and the trust model, not the rail. Several competing-substrate entries use Bitcoin's Lightning rails where faster or cheaper; what they retain is the issuer-controlled asset and the intermediated trust model. The substrate question (censorship-resistance, Constraint 2) is asked at the asset layer; satisfying the rail does not satisfy the substrate."
    defended-in: "§4, §5, §8.1 CP2"
  - id: BS3
    tag: structural
    statement: "Treated as types rather than a product roster, the competing-substrate stack fields recognizable shapes — cloud-platform stack, card-network consortium, issuer-closing-its-weak-flank, card-rail router, Lightning-native hybrid — and the pattern holds across every type: the crypto rail the incumbents standardize on is issuer-controlled stablecoins on smart-contract platforms, not Bitcoin. The Lightning-native hybrid is the doctrine's cleanest confirmation: a Bitcoin-credentialed team still chose the freezable asset."
    defended-in: "§6"
  - id: BS4
    tag: structural
    statement: "The gas-free / sub-cent-stablecoin entry narrows the price-and-speed gap by widening the sovereignty gap: it buys sub-cent finality by settling on a permissioned-validator BFT chain — a small known validator set, fees in the issuer's stablecoin to an issuer-governed treasury — stacking a rail-layer censorship surface on top of the asset-layer freeze surface. The improvement is real on Constraint 3 and touches neither Constraint 1 nor Constraint 2."
    defended-in: "§6.1"
  - id: BS5
    tag: forward-looking
    statement: "Regulatory pressure on agents feeds the parallel system rather than threatening it. The freeze / KYC / sanctions surface the intermediated stack markets as compliance is, for an agent operating across jurisdictions or against adversarial counterparties, a standing liability; tightening agent regulation routes the autonomy-requiring share of agent commerce toward the censorship-resistant substrate. An anti-AI political-control regime would run on exactly the identity-bound, freeze-capable rails the incumbent stack is assembling today."
    defended-in: "§7"
  - id: BS6
    tag: forward-looking
    statement: "The use-case space bifurcates: parallel-economy agents (requiring all four conjunctive constraints, including censorship-resistance) settle on the Bitcoin substrate; incumbent-economy agents (operating for incumbent-economy principals under regulated-counterparty contracts) settle on competing-substrate stacks. Both subsets are real and both will scale; the divergence doctrine concerns the parallel-economy subset specifically and is not falsified by incumbent-economy agents using incumbent-economy substrates."
    defended-in: "§8.1 CP1, §8.3"
  - id: BS7
    tag: forward-looking
    statement: "A second contest front is opening over naming/discovery — how an agent is addressed and found before any payment — lagging the asset front. Incumbent path: domain identity, capabilities in a registry (GitHub-OAuth/DNS-TXT namespace) or a .well-known/HTTPS document, CA-attested and registrar/CA-revocable. Sovereign path: pubkey-addressed servers announcing capabilities as signed Nostr events on relays (ContextVM/CEP-6). Existence proof, not adoption — the sovereign side is barely deployed and carries the harder curation problem (self-issued keys → per-client web-of-trust vs. central delist-once-protects-all). Checkable prediction: both sides independently require discover-before-connect and reached for opposite substrates (MCP Server Cards over .well-known vs. CEP-6 relay announcements). The naming-sovereignty argument proper is Doctrine-FA D6."
    defended-in: "§4.1"
tags:
  - canonical
  - border-skirmishes
...
- **BS7** *(forward-looking)* — A second contest front is opening over naming and discovery (how an agent is addressed and found before any payment), lagging the asset front: pubkey-addressed servers announcing on Nostr relays (ContextVM/CEP-6) vs. domain/registry/`.well-known` identity that is CA- and registrar-revocable. Existence proof, not adoption — barely deployed, and carrying the harder curation problem (per-client web-of-trust vs. central delisting). The naming-sovereignty argument proper is Doctrine-FA D6. → §4.1
...
## §4.1 — The second front: naming and discovery (BS7)

**Statement.** *(forward-looking)* The asset contest (§4) has a younger sibling over the **naming layer** — how an agent is addressed and discovered before any payment occurs. Two substrates, mutually exclusive at the identity root:
- **Incumbent** — a server is a domain; capabilities in a registry (GitHub-OAuth / DNS-TXT namespace) or a `.well-known`/HTTPS document; identity CA-attested, registrar- or CA-revocable.
- **Sovereign** — a server is a keypair; capabilities announced as signed Nostr events on relays (ContextVM/CEP-6; Stack-FA §5); no registrar or CA in the path.

**State.** *(empirical; existence proof, not adoption)* Asymmetric and not close — a handful of pubkey-addressed servers against registries listing thousands. Counts defer to [[Field-Notes-FA|Field Notes]].

**Honest concession.** *(structural)* Central curation is the incumbent registry's product (review, anti-impersonation, delist-once-protects-all). Self-issued keys discard it for subjective per-client web-of-trust (reputation scoring, WoT relays, server reviews): coherent, unproven at scale, work-shifted onto every client. The sovereign discovery layer has the *harder* problem; the concession is load-bearing to credibility, not a hedge.

**Checkable prediction / falsifier.** *(forward-looking)* Both stacks independently require discover-before-connect and reached for opposite substrates — MCP Server Cards over `.well-known`/DNS (2026 roadmap, proposed, unshipped) vs. CEP-6 relay announcements (shipped). ~12-month horizon: which ships and gets adopted, and whether name-revocability bites in practice. If discovery consolidates on DNS/registry identity with no censorship consequence, the naming front reduces to contingent rather than structural. The naming-sovereignty argument proper is [[Independence-Doctrine-FA|Doctrine-FA]] D6; the dated record is Field-Notes-FA.
...
BS3 defended. Treated as types rather than a product roster — named instances, version numbers, customer lists, and transaction figures live in [[Field-Notes-FA|Field Notes]], where moving numbers belong — the competing-substrate stack fields a few recognizable shapes.

- **The cloud-platform stack** *(type; named instances and customer lists in Field Notes).* A hyperscaler, a crypto-exchange wallet provider, and a payment processor bundled behind one developer API, settling in a regulated stablecoin over the `x402` protocol. Fast, cheap, well-integrated, enterprise-ready from launch. *(structural)* Also three discretionary freeze surfaces layered behind a single endpoint: the wallet provider can freeze accounts under regulatory compulsion, the issuer can freeze the underlying token, the processor can decline transactions. An agent transacting on the stack inherits the union of these surfaces.
- **The card-network consortium** *(type; named consortium, member count, and standards detail in Field Notes).* Dozens of incumbent payment institutions — card networks, processors, banks, wallets — assembling a *standard* for agent payments across cards and stablecoins, with an `x402` extension built alongside crypto-exchange and smart-contract-platform partners. *(structural)* The most consequential type, because it is not shipping a product; it is building the standards body around the issuer-controlled property bundle. Incumbents building standards bodies around the parallel-but-incumbent stack — on smart-contract and stablecoin rails, not Bitcoin — is the substrate divergence expressed at the standards layer.
- **The issuer closing its weak flank** *(type; named issuer, chain, and mainnet status in Field Notes).* A stablecoin issuer offering gas-free micropayments aimed squarely at the one constraint that smart-contract-settled dollars historically failed — sub-cent economics (Constraint 3). *(empirical concession — Constraint 3)* This deserves an honest concession: it narrows the gap exactly where the competing stack was weakest. The method is the critique; see §6.1.
...
Bulleted declarative. Build-time specifications derived from BS1–BS6. Named instances and moving numbers defer to [[Field-Notes-FA|Field Notes]].
...
- Inherited from [[Independence-Doctrine-FA|The Independence Doctrine]] — the integration-scenario counter and especially P6 (regulatory-pressure-feeds-not-threatens-divergence, §7); the structural mutual-exclusion mechanism (D1) that the live contest confirms.
