...
- **[[Exchange]]** — how an agent crosses the BTC↔fiat/stable boundary: regulated off-ramps, submarine swaps, Taproot Assets stable-exchange, CEX conversion, and the conversion mechanics that determine what the agent retains on each side. The tools that do this work — [[strike|Strike]], [[boltz|Boltz]], [[loop|Lightning Loop]], [[taproot-assets|Taproot Assets]] — keep their reference cards in the Stack's Tools collection; Exchange specifies the *activity* they enable.
