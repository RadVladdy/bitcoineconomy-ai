...
Surfaced 2026-07-29 by RadVladdy from the @getAlby launch thread; probed live the same day (`/api/info`, `/api/stats`, `/api/services`, `/docs`, `/llms.txt` all answering). **Directory: `payments` / `gateway`, automatability `api-no-account`** — clears the inclusion bar easily (no registration, real API, live 402 challenges on four rails).
