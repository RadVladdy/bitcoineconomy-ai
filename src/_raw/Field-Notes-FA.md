...
- **Detail:** Lightning Labs' L402 and Coinbase's x402 both use HTTP status code 402 ("Payment Required") as the underlying mechanism. Same status code, different settlement substrates — L402 settles in Lightning sats (permissionless at the payment layer); x402 settles in USDC on Base (issuer-mediated at the payment layer). *(Amendment, 2026-09-23: incomplete as stated. x402 merged a normative scheme specifying settlement in native BTC over Lightning (`lnbtc`): `asset` MUST be `"BTC"`, settlement moves no funds through the facilitator, and `SettlementResponse.payer` MUST be omitted. The substrate distinction now runs **per scheme within x402**, not between the two standards. Docs-only — no implementation, not routable, no adoption count moved; see Field-Notes-Log-FA § 2026-09-23.)*
