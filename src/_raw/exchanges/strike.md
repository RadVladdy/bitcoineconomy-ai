...
links-verified: 2026-08-07 (API docs read, not just resolved; auth corrected to API key; fees/exact-coverage deferred to the venue)
---
...
**Dependencies & payment.** A KYC'd Strike account (the compliance boundary) and its public REST API, authenticated with a dashboard-generated API key (OAuth access tokens are the separate path for third-party delegated access). Fund from a bank or card, or receive over Lightning; convert Lightning↔fiat through the API; withdraw BTC to self-custody promptly. (USDT support is TRON-network-only and regional.)
