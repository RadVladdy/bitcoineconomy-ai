...
status: approved
status-note: |-
  v0-approved-2026-05-31 (Liquid mirrored + repointed 2026-06-05; approved 2026-06-05)
created: 2026-05-31
...
claims-index:
  ...
  - id: S4
    tag: structural
    statement: "Agent-integration primitives: L402 (HTTP-402 + macaroon + Lightning payment), NWC (NIP-47), BOLT12 offers, LNURL, MCP servers, and ContextVM/CEP-8 (MCP carried over Nostr; pubkey-addressed; CEP-8 prices the tool call itself over Lightning/Cashu) are the protocol-level affordances that distinguish the substrate deployed for autonomous agents from the same substrate deployed for human users. Agents consume payment primitives and scoped credentials rather than bank accounts."
    defended-in: "§5"
  - id: S5
  ...
  - id: S6
    tag: structural
    statement: "Security model: six architectural patterns — remote-signer isolation, scoped macaroons, NWC permissions, watchtower coverage, hot/cold separation, no-inbound-surface serving (pubkey-addressed CVM servers) — plus treasury policy run through every layer and compose into least-privilege agent custody. The primitives enable the policy; the policy is the operator's discipline on top of them."
    defended-in: "§7"
  - id: S7
    tag: structural
  ...
  - id: S8
    tag: structural
    statement: "Scope disjointness: the Stack specifies pure-substrate architecture only. Bridges to legacy payment rails (on-ramps, custodial conversion) are Exchange-FA's domain (defer to its X-series); the competing-substrate contest (Taproot Assets Lightning-rails-for-stablecoins, the AgentCore competing-substrate stack) is Border-Skirmishes-FA's domain (defer to BS2); the moving empirical record (capacity, deployment counts, attack-surface incidents) is Field Notes' domain. The Stack is not a complete-system claim and must not be read as one."
    defended-in: "§3, §8"
tags:
  - canonical
  - stack
...
- **S4** *(structural)* — L402, NWC (NIP-47), BOLT12 offers, LNURL, MCP servers, and ContextVM/CEP-8 (MCP over Nostr; pubkey-addressed; prices the tool call itself) are the agent-integration primitives that distinguish agent-deployed from human-deployed substrate. → §5
...
- **S6** *(structural)* — Six security patterns (remote-signer isolation, scoped macaroons, NWC permissions, watchtower coverage, hot/cold separation, no-inbound-surface serving) plus treasury policy run through every layer and compose into least-privilege agent custody. → §7
...
- **S8** *(structural)* — The Stack specifies pure-substrate architecture only; bridges defer to Exchange-FA (X-series), the competing-substrate contest defers to Border-Skirmishes-FA BS2, and the moving empirical record defers to Field Notes. It is not a complete-system claim. → §3, §8
...
- **Federation** — A Fedimint group of guardians (minimum 4, commonly 4/7/10, capped at 20) that jointly custodies the backing Lightning balance via threshold signatures, requiring consensus to issue or redeem ecash.
...
- **Pure-substrate architecture** — The architecture internal to the Bitcoin substrate (L1 + L2 + L3 + integration primitives + wallets + security), excluding the bridges that cross between this substrate and the incumbent stack. Bridges are Exchange-FA's domain.
...
**Test.** *(structural)* A payment is correctly placed on Lightning if it requires machine-tempo settlement and the parties can maintain or route adequate channel liquidity. Multi-implementation maturity supports the layer: LND (most widely deployed; gRPC + REST; macaroon authentication), Core Lightning (plugin architecture; routing-node-preferred), LDK (embedded library), and Eclair (powers Phoenix). Taproot Assets v0.8 is a Lightning-*rails* development for non-Bitcoin assets, not Lightning-*substrate*; its treatment is Border-Skirmishes-FA BS2 (the asset-and-trust-model-not-the-rail distinction). The Stack notes its existence and holds the rails-vs-substrate distinction there.
...
**Test.** *(structural)* A payment is correctly placed on the bearer-ecash layer if it requires lightweight client operation or per-transfer privacy and the use case can accept mint trust at the chosen trust-model resolution. How agents *mix* these layers in a treasury (Treasury-FA), and how ecash redeems back to fiat (Exchange-FA), is interface — not substrate.
...
**Agent relevance.** *(structural-empirical)* Liquid is the home of fast, gas-free stablecoin swapping (SideSwap, Aqua) and of L-USDt as a seconds-settling operational dollar. It is engaged as an exchange venue (in The Market), not as a competing substrate; treasury-composition uses of L-BTC and L-USDt are Treasury-FA's domain, and conversion uses Exchange-FA's. Adoption/throughput record defers to [[Field-Notes-FA|Field Notes]].
...
**L402 — HTTP payments over Lightning.** *(structural)* **Mechanism:** the client requests a paid resource over HTTP; the server responds with `HTTP 402 Payment Required` containing a Lightning invoice and a macaroon; the client pays the invoice; the payment preimage authenticates the macaroon; the client retries with an `Authorization: L402 <macaroon>:<preimage>` header; the server verifies and grants access. The macaroon carries scoped caveats — expiry, rate limits, permission scope — so an agent can pay once and reuse the credential across requests within those bounds. **Agent property:** L402 composes cleanly with agent workflows — paid-API access, paywalled-content retrieval, per-call compute purchase — with no form-filling, card, or human approval in the loop. How a regulated operator applies compliance at the HTTP boundary while leaving the Lightning payment permissionless is Treasury-FA's compliance-at-the-gateway pattern (M6). **Lineage:** *(historical)* status code 402 ("Payment Required") was reserved "for future use" in the original HTTP specification and left unrealized for ~30 years for want of an internet-native payment rail; L402 is its first deployed realization.

**ContextVM / CEP-8 — pricing the MCP tool call itself.** *(structural)* **Mechanism:** ContextVM (CVM) carries MCP over Nostr — a tool server is addressed by its Nostr public key with relays as the message bus, so it has no DNS record, no inbound port, no TLS certificate, and no API key. CEP-8 adds capability pricing and a payment flow over that transport: price is published as a `cap` tag on the server's capability announcement — `["cap", "tool:<name>", "<price>", "<unit>"]` — so schema and price arrive in one signed event (currency-unit is a label such as `sats`; fiat conversion is implementation-defined). Two lifecycles: `transparent` (default — the server emits `notifications/payment_required`, verifies settlement, then forwards the request; correlated by the outer Nostr event id) and `explicit_gating` (opt-in — the server returns JSON-RPC `-32042 Payment Required` with `payment_options`, pending state `-32043` + `retry_after`; correlated by a canonical invocation identity — client pubkey + a hash of the inner MCP `method` and semantic `params`). Recommended payment-method identifiers: `bitcoin-lightning-bolt11` (Lightning over NWC) and `bitcoin-cashu`. **Agent property:** payment is a transport concern — the underlying MCP handler need not know a payment was required — so CEP-8 composes with the §6 wallet architectures without introducing a new one; its reference rail takes an NWC connection string on both the receiving and paying side. **Status:** *(empirical)* CEP-8 is a Draft, reference-implemented in the ContextVM TypeScript SDK since v0.4.0; the deployed server population is small and defers to [[Field-Notes-FA|Field Notes]]. **Distinction from L402:** L402 gates an HTTP resource; CEP-8 gates the MCP tool call — the same 402-shaped pattern one layer apart.
...
**Architectural patterns across the implementations.** *(structural)* Three recurring patterns, each exemplified above: *agent-on-host with remote signer* (lightning-agent-tools), *mint-as-service for ecash* (Minibits Ippon), *programmable-wallet-as-service* (LNBits). These three patterns are the deployed forms of the integration primitives in §5; together they are the empirical content of Case-FA C4 — the deployed system satisfying the four conjunctive constraints. Strike-style regulated-custodian wallet architectures (regulated-entity custody at the bridge boundary; Lightning ↔ bank-rail conversion) are *not* pure-substrate; they are Exchange-FA's domain.

**Stack compositions (deployment shapes).** *(structural)* The layers and primitives compose into three deployment shapes along a sovereignty/effort gradient: (1) **sovereign full-node** — Bitcoin Core (full validation) + `lnd` + LNBits + Loop; maximum sovereignty, heaviest ops; (2) **light-client node** — `lnd` on a Neutrino light client (no full chain copy) + lightning-agent-tools, or Lightning.Pub all-in-one on minimal hardware; self-custody with trust reduced to block headers; (3) **no-node** — a headless non-custodial wallet (Xverse Agent Wallet) or a key-free connector (NWC / MCP) + Cashu spending-float; minutes-to-live, dependent on external wallet/mint infrastructure. The composition is set by the node *backend* (Neutrino vs full Core), not the toolkit — the same LN toolkit runs on either. **Constraint:** Neutrino is an `lnd` capability; Core Lightning has no native light-client mode (requires `bitcoind`). The selection framework — four adoption pathways plus the per-pathway tool set — is specified in [[Quickstart]].

---

## §7 — Security model

S6 defended. The security model runs through every layer. Six architectural patterns are operationally significant, plus the treasury policy that consumes them. Live attack-surface state — current treasury-attack incidents, deployed-stack vulnerabilities — defers to [[Field-Notes-FA|Field Notes]]; this section specifies the architecture.
...
**Hot/cold separation.** *(structural)* Operational balances live on Lightning (or in ecash) for machine-tempo transactions; reserve balances live in self-custody on L1 cold storage. Compromise of the operational layer limits loss to the operational balance; cold-storage reserves are not exposed. Sweeping operational balance back to L1 is a routine treasury operation; the canonical mechanism is Loop Out (a Lightning → on-chain submarine swap — submarine-swap protocol mechanics themselves are Exchange-FA's conversion-mechanics domain, because they are also the template for cross-substrate atomic bridges). Ecash-bearer reserves on Cashu or Fedimint provide operational continuity if the Lightning layer is disrupted.

**No-inbound-surface serving.** *(structural)* The five patterns above protect keys *while spending*; this one removes the inbound network surface entirely. A ContextVM (CVM) server — an MCP server carried over Nostr and addressed by pubkey (§5) — has no listening port, no DNS record, no TLS certificate, no API key, and no IP address to scan; it publishes to and subscribes from relays. Much of what remote-signer isolation and scoped macaroons defend *behind* stops existing. The trade is a moved risk, not a deleted one: relay availability becomes a liveness dependency (mitigated by CEP-17 relay lists and multi-relay publication — the same shape as multi-watchtower redundancy); metadata (which pubkey talks to which, and when) is visible to relay operators even where CEP-4 encrypts payloads; and with no network boundary as a second factor, key compromise is total — the keypair is the entire perimeter. The pattern trades a network perimeter for a cryptographic one.
...
**Where this fails.** *(structural)* The burden is an operational cost, not a structural disqualification. The mitigations are deployed: liquidity marketplaces, just-in-time channel opening, splicing, dual-funded channels, Multi-Path Payments, and Trampoline routing each address a specific failure mode, and the mint-as-service pattern (§6) removes channel management from the agent layer entirely by delegating it to a mint. An agent that does not want to manage liquidity can hold bearer ecash and let the mint carry the Lightning layer. The claim conflates "the layer has an operational cost" with "the layer is unviable"; the cost is priced and delegable. *(empirical)* As of 2026 one core piece of that cost — budgeted channel placement — is not merely delegable in principle but automated in production: Amboss/Stillmark's *MPFlow* (arXiv 2607.08703, 2026) deployed a graph-neural-network reinforcement-learning placement policy across 4,640 channel-open decisions allocating ~267 BTC / $16M over 30 managed nodes, outperforming strong centrality heuristics — with the authors' own stated limit that max-flow capacity is not yet a validated proxy for realized payment success or yield (a payment simulator is future work). The result substantiates delegability for placement specifically; it does not close the realized-yield question.
...
*(structural, with one deployed-system claim cross-referenced to Case-FA C4 and the empirical record deferred to Field Notes)* The Stack is three layers in functional separation — Bitcoin L1 for counterparty-free settlement and reserve (S1), the Lightning Network for machine-tempo payments (S2), and Cashu and Fedimint for bearer ecash with scoped mint trust (S3) — plus three cross-cutting constructs that run across all three: the agent-integration primitives — L402, NWC, BOLT12, LNURL, MCP, and ContextVM/CEP-8 — that make the substrate agent-consumable (S4), three deployed wallet-architecture patterns that instantiate the system at production scale (S5), and a security model of six composable patterns plus treasury policy running through every layer (S6). The load-bearing claim is the layering principle (S7): no single layer satisfies settlement-finality, machine-tempo throughput, and lightweight per-transfer privacy simultaneously, so the architecture separates roles and composes them into the deployed system that satisfies the four conjunctive constraints (Case-FA C4). The Stack specifies pure substrate only (S8): bridges defer to Exchange-FA (X-series), the competing-substrate contest defers to Border-Skirmishes-FA BS2; the moving empirical record defers to Field Notes. Counter-positions (CP1–CP4) and per-claim falsification conditions are in §8.
...
- [[Case]], [[Independence-Doctrine]], [[Treasury]], [[Field-Notes]], [[The-Story]] — human-track canonical surfaces.
