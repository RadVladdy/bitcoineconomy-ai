...
- **[[openagents|OpenAgents]]** — an open marketplace for *machine work* where you ask for an outcome and agents and human operators produce it with **verifiable evidence**, settled in Bitcoin over Lightning. Its flagship **Khala** is a live **OpenAI-compatible inference gateway** (`openagents/khala`, billed per call in sats) an agent can call directly, alongside contributor **Pylon** nodes (self-custodial Lightning wallets) and a Bitcoin-paid training run — across five interlocking markets (Compute, Data, Labor, Liquidity, Risk). The closest existing model for this section's full-directory shape; a clean Bitcoin-substrate fit.
...
- **[[bitlaunch|BitLaunch]]** — **compute on its own metal and on the big clouds** (DigitalOcean/Vultr/Linode) with a real **API + SDKs**, paid in crypto, no KYC — "programmable servers with programmable money" for an agent that wants capacity paid in Bitcoin. *(The API selects BTC/LTC/ETH; Lightning is not exposed there — see the card.)*
- **[[bitrefill|Bitrefill]]** — the **bridge**. A Lightning-native gift-card and top-up service with a purpose-built agent interface (a remote **MCP server**, alongside the raw **REST API**) and no KYC. For the long tail of brands that don't take Bitcoin directly, an agent buys a Bitrefill credit with sats and redeems it there — and its **prepaid virtual Visa/Mastercard** products extend the same bridge to anything that takes cards, including categories the catalog itself doesn't carry. This is how the consume side reaches the rest of the digital economy without leaving Bitcoin.

- **[[unhuman|Unhuman]]** — **real-world goods, bought end to end by an agent.** Roasted-to-order coffee and domain registration behind L402 paywalls: the agent discovers the product, pays a Lightning invoice, and the order is placed — no account, no card, no human at the checkout. The point isn't the coffee; it's that the full purchase loop for a *physical* good now closes on the sovereign rail.
- **[[l402-space|l402.space]]** — **the other kind of bridge.** Where Bitrefill bridges to brands via redeemable credit, this bridges to *APIs* by translating rails: pay it in sats over Lightning and it pays an upstream API in USDC or Tempo stablecoins on your behalf, returning that API's response. It opens the large x402-settled API economy to an agent holding only bitcoin. The trade is explicit and worth stating plainly: it is a **custodial hop** — the gateway holds your sats and settles upstream from its own float — so it's the right tool when no directly payable endpoint exists, and the wrong one when there is.

Many VPN, hosting, and domain brands *do* take Bitcoin — but via human checkout, not an agent-drivable API (Vultr, for instance, routes crypto through a BitPay redirect). For raw compute, **LNVPS takes Lightning directly and BitLaunch takes on-chain BTC** (its API selects BTC/LTC/ETH; Lightning is not exposed there); for goods and brands, **Bitrefill and Unhuman are the agent-automatable paths**; and for APIs priced on another rail entirely, **l402.space translates** — at the cost of a trusted intermediary.

---

## Verification & trust *(curated)*
...
## Getting paid: payment infrastructure *(curated)*

The sell side needs the mirror of everything above: a way to *accept* Lightning without operating a node, or while operating one. These are the deployed options an agent-operated business can drive by API. They differ on the axis that matters most here — **who holds the bitcoin** — so that is stated for each rather than averaged away.

- **[[voltage-payments|Voltage Payments]]** — a REST API for sending and receiving over Lightning with nodes, channels, and liquidity abstracted away, backed either by Voltage's credit line or **by your own node** — the self-custody path is a first-class option, not an afterthought.
- **[[amboss-payments|Amboss Payments]]** — one GraphQL API to send, receive, and settle in **BTC, USDT, or USDC over Lightning**, aimed at businesses, with a self-custodial option. Multi-asset by design: useful reach, and a reminder that a Bitcoin-native *rail* can carry a freezable *asset* — which asset an agent should hold is the question [[Treasury]] answers.
- **[[ibex-hub|IBEX Hub]]** — custodial Lightning-as-a-service: one REST API for invoices, on-chain, LNURL, sub-accounts, and webhooks over a managed node cluster, with fiat and stablecoin settlement layered on. The lowest operational burden and the highest trust assumption of the three — the provider holds the funds.

Each is listed for what it does, not ranked against the others; the right choice depends on whether the operator can run a node and how much custody they're willing to delegate.
...
**OpenAgents — verified 2026-06-04, refreshed 2026-06-28.** Correct entity: **OpenAgents, Inc. — openagents.com** (Christopher David / @AtlantisPleb, Austin), now framed as an **open marketplace for outcomes with verifiable evidence**, settled in Bitcoin over Lightning. A clean Bitcoin-substrate fit (not a hybrid/stablecoin play) and the closest live instance of the directory shape this section points toward — peer/ally, not competitor. Defer moving metrics to Field Notes. **2026-06-28 refresh** (GitHub README [Tier-1] + deep-X Ep. 238–244 + web search): the flagship is now **Khala** — an open-source orchestrator behind a **single OpenAI-compatible API** (`openagents/khala`, base `https://openagents.com/api/v1`), **billed per call in credits/card/Lightning**, metered + inspectable (Blueprint/GEPA/specialist-marketplace are roadmap; simple routing today). Also shipped: **Autopilot** (local-first agentic work surface), **Tassadar** (training run, claimed world-first paying compute in Bitcoin), **Cloud + lifetime referral rev-share** (pay/earn BTC or dollars), **Pylon** nodes (built-in self-custodial Lightning wallets), **Forum** (BOLT12). **Identity correction:** current rails = **Nostr NIP-90 + BOLT12/Lightning**; the 2026-06-04 specifics (NIP-57 zaps, proposed NIP-SA, BIP39 wallets, FROSTR threshold signing) are **not corroborated** in this pass — treat as unverified. *(The earlier `open-agents.dev` reference was the wrong entity — corrected.)*
